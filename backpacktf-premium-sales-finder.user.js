...
// @downloadURL https://github.com/yaboieeek/backpack.tf-premium-sales-finder/raw/master/backpacktf-premium-sales-finder.user.js
// @run-at      document-end
// @grant       GM_addStyle
// @grant       unsafeWindow
// @grant       GM_getValue
...
const selectors = {
    items: `.premium-search-results .result`
}
...
class UIController {
    constructor(itemsList = null) {
        this.itemsList = itemsList;
        this.buttonState = GM_getValue('_buttonState') ? GM_getValue('_buttonState') : buttonStates.all;
    }

    init() {
        this.itemsList === null && this._fromPage();
        this._toggleItemsVisibility();
        this.createControls();
    }

    createControls(){
        this._createToggleVisibilityButton();
    }

    _fromPage() {
        this.itemsList = [...document.querySelectorAll(selectors.items)];
        console.log(`Initialized ${this.itemsList.length} items!`);
    }

    _createToggleVisibilityButton() {
        const target = document.querySelector('#build-premium-search');
        const buttonContainer = document.createElement('div');
        buttonContainer.className = 'eeek-button-container';
        const button = document.createElement('button');
        button.className = 'eeek-toggler-button';
        button.type = 'button';
        ...
        button.addEventListener('click', () => this._buttonAction(button));
    ...
    _changeButtonState(){
        this.buttonState = this.buttonState === buttonStates.all ? buttonStates.sales : buttonStates.all;
        GM_setValue('_buttonState', this.buttonState);
    }
